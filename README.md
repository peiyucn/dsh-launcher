...
* **Source run** — clones deepseek-harness automatically into a managed location and runs it (a custom `dsh.path` overrides the clone location). On first start it also runs `pnpm install` + build, using dsh's official build profile so the web UI shows the same DeepSeek Harness brand as the packaged dsh.
...
* Log files: `%LOCALAPPDATA%\dsh-launcher-panel\logs\client.log` (launcher activity) and `server.log` (server output), alongside the managed install/source dirs; both are clickable in the panel.
